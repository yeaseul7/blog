...
CRA \`\`\`Create React App\`\`\` 은 기본적으로 SSR \`\`\`Server Side Rendering\`\`\` 을 지원하지 않고 CSR \`\`\`Client Side Rendering\`\`\` 에 초점을 맞췄다.
...
*(사전에 나는 Next.js가 SSR만 제공하는 줄 알았으나 SSG도 지원한다는 사실을 알게 되었다.)*

\`\`\`
React: CSR(Client-Side-Rendering)
Next.js: SSR(Sever-Side-Rendering), SSG(Static-Site-Generation)
\`\`\`
*개인 블로그를 만들 때 Vuepress를 사용하는 것도 괜찮은 방법 같다.*
